...
class LandscapingApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.upload_server = None  # Lazy load when upload feature is used
        self.upload_temp_dir = None  # Temporary directory for uploads
        ...
        self.main_container.grid_rowconfigure(2, weight=0)  # Status bar row
    ...
    def get_upload_server(self):
        """Get upload server with lazy initialization."""
        if self.upload_server is None:
            # Create temp directory for uploads
            if self.upload_temp_dir is None:
                self.upload_temp_dir = tempfile.mkdtemp(prefix="landscaping_uploads_")

            # Initialize upload server with callback
            try:
                self.upload_server = UploadServer(
                    upload_callback=self.handle_uploaded_image,
                    port=5000
                )
            except ImportError:
                return None
    ...
    def handle_uploaded_image(self, filename: str, image_data: bytes):
        """
        Handle an uploaded image from phone.
        ...
        try:
            ...
            temp_path = os.path.join(self.upload_temp_dir, filename)
            with open(temp_path, 'wb') as f:
                f.write(image_data)

            # Show status
            self.show_status(f"Received {filename}, processing with OCR...", "info", 0)

            # Process with OCR in the background (to avoid blocking)
            def process_ocr():
                try:
                    ocr_scanner = self.get_ocr_scanner()
                    result = ocr_scanner.scan_and_parse_visits(temp_path, debug=True)

                    # Update UI on main thread
                    self.after(0, lambda: self._handle_ocr_result(result, filename))
                except Exception as e:
                    self.after(0, lambda: self.show_status(
                        f"OCR failed for {filename}: {str(e)}", "error", 10000
                    ))
            ...
            self.show_status(f"Failed to save upload: {str(e)}", "error", 10000)

    def _handle_ocr_result(self, result: dict, filename: str):
        """Handle OCR result on main thread."""
        if result['success'] and result['records']:
            self.show_status(
                f"✓ Found {len(result['records'])} visit(s) in {filename}",
                "success",
                5000
            )
            # Show the verification dialog
            self.show_scanned_visits_dialog(
                result['records'],
                result['text'],
                self.get_ocr_scanner()
            )
        else:
            error_msg = result.get('error', 'Unknown error')
            self.show_status(f"OCR processing failed: {error_msg}", "error", 10000)
    ...
    def create_status_bar(self):
        """Create the status/action bar at the bottom of the window."""
        self.status_bar = ctk.CTkFrame(self.main_container, height=35, fg_color=("gray85", "gray20"))
        ...
        # Status icon (for success/error indication)
        ...
        self.status_icon.grid(row=0, column=1, padx=5)
    ...
    def show_status(self, message: str, status_type: str = "info", duration: int = 5000):
        ...
        # Clear status after duration (if duration > 0)
        if duration > 0:
            self.after(duration, lambda: self._clear_status())
    ...
    def add_new_client(self):
        ...
        def save_new_client():
            ...
            self.show_status(f"Client '{name}' added successfully!", "success")
            dialog.destroy()
            self.refresh_clients_list()
            self.refresh_dashboard()
    ...
    def save_client_changes(self, client_id: int):
        ...
        try:
            ...
            self.show_status("Client updated successfully!", "success")
            self.refresh_clients_list()
            self.refresh_dashboard()
        except Exception as e:
    ...
    def deactivate_client(self, client_id: int):
        ...
        if messagebox.askyesno("Confirm", "Deactivate this client? They will be hidden from active views."):
            ...
            self.show_status("Client deactivated", "success")
            self.refresh_clients_list()
            self.refresh_dashboard()
            self.show_client_placeholder()
    ...
    def activate_client(self, client_id: int):
        ...
        self.show_status("Client reactivated", "success")
        self.refresh_clients_list()
        self.refresh_dashboard()
        self.show_client_details(client_id)
    ...
    def add_material_to_client(self, client_id: int):
        ...
        def save_material():
            ...
            self.db.add_client_material(client_id, selected_material['id'], custom_cost, multiplier)
            self.show_status("Material added to client!", "success")
            dialog.destroy()
            self.show_client_details(client_id)
    ...
    def add_new_visit(self):
        ...
        def save_visit():
            ...
            self.show_status("Visit added successfully!", "success")
            dialog.destroy()
            self.load_client_visits(client_id)
            self.refresh_dashboard()
    ...
    def add_new_material(self):
        ...
        def save_material():
            ...
            self.show_status("Material/service added successfully!", "success")
            dialog.destroy()
            self.refresh_materials_list()
    ...
    def init_import_tab(self):
        ...
        desc = ctk.CTkLabel(
            self.tab_import,
            text="Import client and visit data from Excel files or scan paper records with your phone.",
            font=ctk.CTkFont(size=11),
            text_color="gray"
        )
        ...
        upload_btn = ctk.CTkButton(
            ...
            command=self.show_phone_upload_dialog,
            font=ctk.CTkFont(size=16),
            height=50,
            fg_color=("#3B8ED0", "#1F6AA5")
        )
        upload_btn.grid(row=5, column=0, sticky="ew", padx=50, pady=10)

        instructions = ctk.CTkTextbox(self.tab_import, height=300)
        instructions.grid(row=6, column=0, sticky="ew", padx=20, pady=20)
...
Phone Upload Instructions:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. Click "📱 Upload from Phone"
2. Scan the QR code with your phone's camera
3. Take photos of your paper records
4. Upload directly from your phone
5. OCR will process automatically
6. Review and save the extracted data

OCR Scanning Instructions:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
4. Save verified data to the database
    ...
    def download_excel_template(self):
        ...
        if file_path:
            if self.excel_importer.generate_template(file_path):
                self.show_status(f"Template saved to: {file_path}", "success", 7000)
            else:
                self.show_status("Failed to generate template", "error")
    ...
    def show_phone_upload_dialog(self):
        """Show dialog with QR code for phone uploads."""
        upload_server = self.get_upload_server()

        if upload_server is None:
            self.show_status(
                "Upload server not available. Install Flask with: pip install flask",
                "error",
                10000
            )
            return
        ...
        # Start the server
        if not upload_server.is_running:
            upload_server.start()
            self.show_status("Upload server started", "success", 3000)

        # Create dialog
        dialog = ctk.CTkToplevel(self)
        dialog.title("Upload from Phone")
        dialog.geometry("600x700")
        dialog.transient(self)
        dialog.grab_set()
        ...
        instructions = ctk.CTkLabel(
            dialog,
            text="Scan this QR code with your phone's camera\nto open the upload page",
            font=ctk.CTkFont(size=13),
            text_color="gray"
        )
        ...
        qr_img = upload_server.generate_qr_code()
        ...
        if qr_img:
            ...
            qr_label = tk.Label(dialog, image=qr_photo, bg=dialog.cget("bg"))
            qr_label.image = qr_photo  # Keep a reference
            qr_label.pack(pady=20)

            # Show URL for manual entry
            url = upload_server.get_server_url()
            url_label = ctk.CTkLabel(
                dialog,
                text=f"Or enter this URL on your phone:\n{url}",
                font=ctk.CTkFont(size=12),
                text_color="blue"
            )
            ...
            def copy_url():
                ...
                self.show_status("URL copied to clipboard", "success", 3000)
        ...
        status_info = ctk.CTkLabel(
            status_frame,
            text="✓ Server is running and waiting for uploads\n"
                 "Images will be automatically processed with OCR",
            font=ctk.CTkFont(size=11),
            text_color="green"
        )
        ...
        # Note about keeping dialog open
        note = ctk.CTkLabel(
            dialog,
            text="Keep this window open while uploading from your phone",
            font=ctk.CTkFont(size=10),
            text_color="gray"
        )
        note.pack(pady=(0, 10))
